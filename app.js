...
messagesEl.innerHTML = `<div class="loading-state">جاري تحميل الرسائل...</div>`;
 try {
  const res = await fetch(
    `${API_BASE}/messages/${encodeURIComponent(sessionId)}`,
    {
      cache: "no-store",
      headers: getAuthHeaders()
    }
  ...
  if (!res.ok) throw new Error("Failed messages");
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
      ...
      chatSubtitleEl.textContent = `${sessionId} • 0 رسالة`;
      return;
    }

    messages.forEach((msg) => appendMessageToUI(msg));
    currentLoadedMessageCount = messages.length;
    chatSubtitleEl.textContent = `${sessionId} • ${currentLoadedMessageCount} رسالة`;
    ...
    if (currentSession === eventSession) {
      ...
      if (conv) {
        ...
        chatSubtitleEl.textContent =
          `${currentSession} • ${currentLoadedMessageCount} رسالة`;
      }
    }
...
function appendRealtimeMessage(messageObj) {
  ...
  if (activeSessionId) {
    chatSubtitleEl.textContent =
      `${activeSessionId} • ${currentLoadedMessageCount} رسالة`;
  }
}
