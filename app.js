...
const SAVED_REPLIES_STORAGE_KEY = "saved_replies";
let savedReplyEditingId = null;

function loadSavedReplies() {
  try {
    const raw = localStorage.getItem(SAVED_REPLIES_STORAGE_KEY);
...
function persistSavedReplies(list) {
  localStorage.setItem(SAVED_REPLIES_STORAGE_KEY, JSON.stringify(list));
}
...
function renderSavedReplies() {
  ...
  replies.forEach((reply) => {
    ...
    deleteBtn.addEventListener("click", (event) => {
      ...
      const remaining = loadSavedReplies().filter((item2) => item2.id !== reply.id);
      persistSavedReplies(remaining);
      renderSavedReplies();
    });
...
saveSavedReplyBtnEl?.addEventListener("click", (event) => {
  event.stopPropagation();
  ...
  if (savedReplyEditingId) {
    const target = replies.find((item) => item.id === savedReplyEditingId);
